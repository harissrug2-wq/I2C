...
function coldStartPayScore(maxDaysOverdue = 0) {
  ...
  if(days>60) return {score:78,label:'Critical delinquency',riskTier:'CRITICAL',provisional:true,confidence:72,basis:`No payment history; oldest open invoice is ${days} days overdue`};
  if(days>30) return {score:60,label:'Concerning delinquency',riskTier:'HIGH',provisional:true,confidence:64,basis:`No payment history; oldest open invoice is ${days} days overdue`};
  if(days>0) return {score:45,label:'Early delinquency',riskTier:'MEDIUM',provisional:true,confidence:55,basis:`No payment history; oldest open invoice is ${days} days overdue`};
