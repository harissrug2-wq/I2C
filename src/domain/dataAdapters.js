...
export function buildEngineInputs(workspace) {
  ...
  const invoiceBalance = i => {
    // Latest manual-data workbook rule: PAID invoices are never A/R, even if
    // an imported file contains total/balance values or no payment allocation.
    if (statusClosesBalance(i.status)) return 0;
    const total = amountOrZero(i.total);
    if (receiptAllocations[i.invoice_no] != null) return Math.max(0, total - receiptAllocations[i.invoice_no]);
    if (hasValue(i.balance_due)) return amountOrZero(i.balance_due);
    // Flexible CSV imports may omit balance_due. For an unpaid invoice the
    // best available balance is the invoice total.
    return total;
  };
  const billBalance = b => {
    if (statusClosesBalance(b.status)) return 0;
    const total = amountOrZero(b.total);
    if (paymentAllocations[b.bill_no] != null) return Math.max(0, total - paymentAllocations[b.bill_no]);
