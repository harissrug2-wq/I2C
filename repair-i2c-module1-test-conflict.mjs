...
const filename={
  ...
  invoiceLines:'invoice_lines.csv',
  bills:'bills.csv',
  paymentsReceived:'payments_received.csv',
  paymentsMade:'payments_made.csv',
  products:'products.csv',
  bankAccounts:'bank_accounts.csv',
  companyMetrics:'company_metrics.csv'
};

const files=datasets.map(dataset=>({
  name:filename[dataset],
  text:workspaceToCsv(dataset,workspace)
}));
...
assert.equal(
  result.workspace.importMeta.completeBundle,
  true,
  'full operational bundle should be marked complete'
);
...
assert.equal(
  engine.cashBalance,
  1284900,
  'cash reconciliation must survive CSV round-trip'
);
...
assert.equal(
  ...
  744790,
  'AR reconciliation must survive CSV round-trip'
);
...
assert.equal(
  ...
  715300,
  'AP reconciliation must survive CSV round-trip'
);

// Best-effort CSV behavior:
// Missing relationships or optional data should warn,
...
assert.equal(
  partial.ok,
  true,
  'missing customer FK must not reject a best-effort manual CSV import'
);

assert.ok(
  partial.warnings.some(e=>e.includes('missing customer')),
  'missing customer warning should be explicit'
);

assert.equal(
  partial.workspace.invoices.some(i=>i.invoice_no==='INV-X'),
  true,
  'available invoice values should still be imported'
);
